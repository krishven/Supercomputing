...
void CFW (int X[][V], int length, int xi, int xj, int ui, int uj, int vi, int vj) {
	if (length == M)
		...
		{
			if (omp_get_thread_num()!=0)	
				CFW(X, newlen, X12, U11, V12);
			else
				CFW(X, newlen, X22, U21, V12);			
		}
		#pragma omp barrier
		...
		{
			if (omp_get_thread_num()!=0)	
				CFW(X, newlen, X11, U12, V21);
			else
				CFW(X, newlen, X21, U22, V12);			
		}
		#pragma omp barrier
	#else
...
void BFW (int X[][V], int length, int xi, int xj, int ui, int uj, int vi, int vj) {
	if (length == M)
		...
		{
			if (omp_get_thread_num()!=0)	
				BFW(X, newlen, X21, U21, V11);
			else
				BFW(X, newlen, X22, U21, V12);			
		}
		#pragma omp barrier
		...
		{
			if (omp_get_thread_num()!=0)	
				BFW(X, newlen, X11, U12, V21);
			else
				BFW(X, newlen, X12, U12, V22);			
		}
		#pragma omp barrier
#else
		BFW (X, newlen, X11, U11, V11);
