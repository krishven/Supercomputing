...
static void scatterData(double *arr, uint64_t arrSize, int rank, int p) {
	...
	qsort(recvBuf,arrSize/p,sizeof(double),compare);
...
//    		printf("%dth node %d:%d\n",i,indexArr[j],indexArr[j+1]);
    ...
    for(i = 0; i < p; i++) {
    	if(i == rank) {
    		...
    		buckets[i] = (bucket_t *) malloc(sizeof(bucket_t)+sizeof(double)*arrSize/p);
  ...
  //  		cout<<endl;
	...
	memset(&bucketSize, 0, sizeof(int)*p);
...
//			cout << recvNum[i] << "\t";
		...
		for (i=1;i<p;i++)
			MPI_Send(&rnum, p*p, MPI_INT,i,0,MPI_COMM_WORLD); 	
	}

	else
		MPI_Recv(&rnum,p*p,MPI_INT,0,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
...
//			printf("ranka:%d\n",rank);
	...
	//bucket_t *tempBucket = (bucket_t *)malloc(sizeof(bucket_t)+sizeof(double)*arrSize/p);
//	cout <<"Rank"<<rank<<"\t";
	for(j=0;j<p;j++)
	{
		if(j!=rank) {
			//MPI_Recv(&tempArray[pos],arrSize/p,MPI_DOUBLE,j,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
			MPI_Recv(&(buckets[rank]->array[(int)buckets[rank]->size]),rnum[rank+(j*p)],MPI_DOUBLE,j,0,
				MPI_COMM_WORLD,MPI_STATUS_IGNORE);
			// MPI_Recv(&(buckets[rank]->array[(int)buckets[rank]->size]),arrSize/p,MPI_DOUBLE,j,0,
			// 			MPI_COMM_WORLD,MPI_STATUS_IGNORE);
			// // memcpy(&(buckets[rank]->array[(int)buckets[rank]->size]),&tempBucket->array[0],sizeof(double)*
			// 	tempBucket->size);
