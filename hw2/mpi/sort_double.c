...
static int binary_search(double num,double *arr,uint64_t l, uint64_t r){
...
static void scatterData(double *arr, uint64_t arrSize, int rank, int p) {
	...
	if(rank == 0) {
		...
		// recvCounts = (double *) malloc(p*sizeof(double));
		// disp = (double *) malloc(p*sizeof(double));
...
*/
	...
	bucket_t *tempBucket = malloc(sizeof(bucket_t)+sizeof(double)*arrSize/p);
	for(j=0;j<p;j++)
	{
		if(j!=rank) {
