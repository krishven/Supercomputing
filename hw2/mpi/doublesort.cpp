...
extern "C++" void qsort(double *a, int n);
...
static void scatterData(double *arr, uint64_t arrSize, int rank, int p) {
	...
	// else
	// 	recvBuf = new double[(arrSize/p)+(arrSize%p)];
	// // 
 ...
 //  		for(i=1;i<p;i++){
	...
	// MPI_Scatterv(arr, arrSize/p, MPI_DOUBLE, recvBuf, arrSize/p,
 //                    MPI_DOUBLE, 0, MPI_COMM_WORLD);

	qsort(recvBuf,arrSize/p);
...
//			cout << buckets[rank]->size <<"\t";
    ...
    qsort(buckets[rank]->array,buckets[rank]->size);
